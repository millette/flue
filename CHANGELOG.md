...
- **Cloudflare shell sandbox moves to a connector install guide.** `flue add cloudflare-shell` installs Workspace helpers including `getShellSandbox()` and `hydrateFromBucket()`. Removed Cloudflare helper imports now throw actionable replacement-code errors.
...
- **Move the Cloudflare shell sandbox helpers out of `@flue/runtime/cloudflare`.** Replace those imports with the file generated by `flue add cloudflare-shell`.
...
- **`local()` Node sandbox factory** exported from `@flue/runtime/node`. It creates a pure-Node host sandbox backed by `node:fs/promises` and `node:child_process`, supports configurable `cwd`, and honors explicit env opt-ins via `local({ env: { ... } })`.
