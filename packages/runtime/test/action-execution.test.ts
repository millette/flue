...
describe('model-called Actions', () => {
	...
	it('inherits a task cwd and selected-profile capabilities when a task calls an Action', async () => {
		...
		const harness = await createContext(provider, store, createNoopSessionEnv({ exec })).init(
			createAgent(() => ({
				model: `${provider.getModel().provider}/${provider.getModel().id}`,
				subagents: [
					{
	...
	it('cancels direct harness shell calls and waits for cleanup before an Action settles', async () => {
		...
		const harness = await createContext(provider, store, env).init(
			createAgent(() => ({
				model: `${provider.getModel().provider}/${provider.getModel().id}`,
				actions: [action],
			})),
...
describe('Action model tools', () => {
	it('rejects one final namespace collision across custom Action and result tools', async () => {
		...
		const harness = await createContext(provider, new RecordingSessionStore()).init(
			createAgent(() => ({
				model: `${provider.getModel().provider}/${provider.getModel().id}`,
				actions: [lookup],
				tools: [
